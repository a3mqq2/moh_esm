...
@section('title', 'إدارة العنايات')

@section('page-title')
    <div class="page-title-head d-flex align-items-center">
        <div class="flex-grow-1">
            <h4 class="page-main-title m-0">إدارة العنايات</h4>
        </div>
        <div>
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="ti ti-plus me-1"></i> إضافة عناية
            </button>
        </div>
    </div>
@endsection
...
@section('content')
            ...
            <div id="help-arrow-1" style="position:absolute; z-index:99991;">
                <div style="background:#fff; color:#333; padding:14px 22px; border-radius:12px; box-shadow:0 8px 32px rgba(0,0,0,0.3); max-width:280px; font-family:Almarai,sans-serif;">
                    <div style="display:flex; align-items:center; gap:8px; margin-bottom:8px;">
                        ...
                        <strong>إضافة عناية</strong>
                    </div>
                    <p style="margin:0; font-size:0.9rem; color:#666;">اضغط <kbd style="background:#040404; color:#fff; padding:2px 8px; border-radius:4px; font-size:0.85rem;">Enter</kbd> لفتح نافذة الإضافة بسرعة</p>
        ...
        <div class="help-step" id="help-step-2" style="display:none;">
            <div id="help-arrow-2" style="position:absolute; z-index:99991;">
                <div style="background:#fff; color:#333; padding:14px 22px; border-radius:12px; box-shadow:0 8px 32px rgba(0,0,0,0.3); max-width:280px; font-family:Almarai,sans-serif;">
                    ...
                    <p style="margin:0; font-size:0.9rem; color:#666;">اكتب اسم العناية ثم اضغط <kbd style="background:#000000; color:#fff; padding:2px 8px; border-radius:4px; font-size:0.85rem;">Enter</kbd> للحفظ مباشرة</p>
                    <div style="margin-top:12px; display:flex; justify-content:space-between; align-items:center;">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0" id="wards-table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>اسم العناية</th>
                            ...
                            <tbody>
                                @forelse($wards as $ward)
                                    <tr id="row-{{ $ward->id }}">
                                        <td>{{ ($wards->firstItem() ?? 1) + $loop->index }}</td>
                                        <td class="ward-name">{{ $ward->name }}</td>
                                        <td>{{ $ward->created_at->format('Y-m-d') }}</td>
                                        <td class="text-center">
                                            <button class="btn btn-sm btn-soft-warning btn-edit" data-id="{{ $ward->id }}" data-name="{{ $ward->name }}">
                                                <i class="ti ti-pencil me-1"></i> تعديل
                                            </button>
                                            <button class="btn btn-sm btn-soft-danger btn-delete" data-id="{{ $ward->id }}" data-name="{{ $ward->name }}">
                                ...
                                @empty
                                    <tr id="no-data">
                                        <td colspan="4" class="text-center text-muted py-4">لا توجد عنايات حتى الآن</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    @if($wards->hasPages())
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            <div class="text-muted small">
                                عرض {{ $wards->firstItem() }} إلى {{ $wards->lastItem() }} من {{ $wards->total() }}
                            </div>
                            {{ $wards->links() }}
                        </div>
                    @endif
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">إضافة عناية جديدة</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form id="addForm">
                    <div class="modal-body">
                        <div class="mb-0">
                            <label for="add-name" class="form-label">اسم العناية <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="add-name" name="name" placeholder="أدخل اسم العناية" required autofocus />
    ...
    <div class="modal fade" id="editModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">تعديل العناية</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form id="editForm">
                    ...
                    <div class="modal-body">
                        <div class="mb-0">
                            <label for="edit-name" class="form-label">اسم العناية <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="edit-name" name="name" placeholder="أدخل اسم العناية" required />
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    ...
                    <p>هل أنت متأكد من حذف العناية <strong id="delete-name"></strong>؟</p>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('addForm').addEventListener('submit', function (e) {
            ...
            .then(function (res) {
                ...
                btn.disabled = false;
                if (res.ok) {
                    var item = res.data.ward;
                    var noData = document.getElementById('no-data');
                    if (noData) noData.remove();
                    var tbody = document.querySelector('#wards-table tbody');
                    var count = tbody.querySelectorAll('tr[id^="row-"]').length + 1;
                    var tr = document.createElement('tr');
                    tr.id = 'row-' + item.id;
                    tr.innerHTML = '<td>' + count + '</td><td class="ward-name">' + item.name + '</td><td>' + item.created_at.substring(0, 10) + '</td><td class="text-center"><button class="btn btn-sm btn-soft-warning btn-edit" data-id="' + item.id + '" data-name="' + item.name + '"><i class="ti ti-pencil me-1"></i> تعديل</button> <button class="btn btn-sm btn-soft-danger btn-delete" data-id="' + item.id + '" data-name="' + item.name + '"><i class="ti ti-trash me-1"></i> حذف</button></td>';
        ...
        document.getElementById('editForm').addEventListener('submit', function (e) {
            ...
            .then(function (res) {
                ...
                if (res.ok) {
                    ...
                    row.querySelector('.ward-name').textContent = res.data.ward.name;
                    row.querySelector('.btn-edit').dataset.name = res.data.ward.name;
                    row.querySelector('.btn-delete').dataset.name = res.data.ward.name;
        ...
        document.getElementById('confirm-delete-btn').addEventListener('click', function () {
            ...
            .then(function (data) {
                ...
                var rows = document.querySelectorAll('#wards-table tbody tr[id^="row-"]');
                if (rows.length === 0) {
                    document.querySelector('#wards-table tbody').innerHTML = '<tr id="no-data"><td colspan="4" class="text-center text-muted py-4">لا توجد عنايات حتى الآن</td></tr>';
                } else {
        ...
        // Enter key opens add modal when no modal is open
        ...
        // Help tour
        var helpStep = 0;
        if (!localStorage.getItem('wards_help_seen')) {
            showHelpTour();
        }
        ...
        window.dismissHelp = function() {
            ...
            var addBtn = document.querySelector('[data-bs-target="#addModal"]');
            if (addBtn) { addBtn.style.zIndex = ''; addBtn.style.boxShadow = ''; }
            localStorage.setItem('wards_help_seen', '1');
            helpStep = 0;
        };
    });
</script>
